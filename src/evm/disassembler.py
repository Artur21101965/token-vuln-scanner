...
def disassemble(bytecode: str) -> list[Instruction]:
    ...
    while i < len(code):
        op = code[i]
        name = opcode_name(op)
        imm_size = push_imm_size(op)
        push_data = code[i + 1 : i + 1 + imm_size] if imm_size > 0 else b""
        instructions.append(Instruction(offset=i, opcode=op, name=name, push_data=push_data))
        i += 1 + imm_size
    return instructions
